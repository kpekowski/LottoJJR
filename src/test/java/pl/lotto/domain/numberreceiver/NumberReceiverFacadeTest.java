...
public class NumberReceiverFacadeTest {
    ...
    public void it_should_return_failed_message_when_user_input_less_than_six_numbers() {
        ...
        // then
        NumberReceiverResponseDto expectedResponse = new NumberReceiverResponseDto(null, ValidationResult.NOT_SIX_NUMBER_GIVEN.info);
        assertThat(response).isEqualTo(expectedResponse);
    }
    ...
    public void it_should_return_correct_draw_date() {
        // given
        Clock clock = Clock.fixed(LocalDateTime.of(2023, 7, 26, 10, 0, 0).toInstant(ZoneOffset.UTC), ZoneId.of("Europe/London"));
        ...
        // then
        LocalDateTime expectedDrawDate = LocalDateTime.of(2023, 7, 29, 12, 0, 0);
    ...
    public void it_should_return_next_Saturday_draw_date_when_date_is_Saturday_noon() {
        // given
        Clock clock = Clock.fixed(LocalDateTime.of(2023, 7, 29, 12, 0, 0).toInstant(ZoneOffset.UTC), ZoneId.of("Europe/London"));
        HashGenerable hashGenerator = new HashGenerator();
        NumberReceiverFacade numberReceiverFacade = new NumberReceiverConfiguration().createForTest(hashGenerator, clock, ticketRepository);
        Set<Integer> numbersFromUser = Set.of(1, 2, 3, 4, 5, 6);
        ...
        // then
        LocalDateTime expectedDrawDate = LocalDateTime.of(2023, 8, 5, 12, 0, 0);
        assertThat(testedDrawDate).isEqualTo(expectedDrawDate);
    }
    ...
    public void it_should_return_next_Saturday_draw_date_when_date_is_Saturday_afternoon() {
        // given
        Clock clock = Clock.fixed(LocalDateTime.of(2023, 7, 29, 15, 0, 0).toInstant(ZoneOffset.UTC), ZoneId.of("Europe/London"));
        HashGenerable hashGenerator = new HashGenerator();
        NumberReceiverFacade numberReceiverFacade = new NumberReceiverConfiguration().createForTest(hashGenerator, clock, ticketRepository);
        Set<Integer> numbersFromUser = Set.of(1, 2, 3, 4, 5, 6);
    ...
    public void it_should_return_tickets_with_correct_draw_date() {
        ...
        Instant fixedInstant = LocalDateTime.of(2023, 7, 27, 12, 0, 0).toInstant(ZoneOffset.UTC);
    ...
    public void it_should_return_empty_collections_if_there_are_no_tickets() {
        // given
        HashGenerable hashGenerator = new HashGenerator();
        Clock clock = Clock.fixed(LocalDateTime.of(2023, 7, 26, 12, 0, 0).toInstant(ZoneOffset.UTC), ZoneId.of("Europe/London"));
        NumberReceiverFacade numberReceiverFacade = new NumberReceiverConfiguration().createForTest(hashGenerator, clock, ticketRepository);
